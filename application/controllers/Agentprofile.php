...
class AgentProfile extends CI_Controller {
    function index(){
        ...
        $this->AgentProfileModel->get_details();
		$this->AgentProfileModel->load_view();
    }
    function update(){
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            $this->load->model('AgentProfileModel');
            $file_path = "";
            if($this->AgentProfileModel->get_details()[0]->image == NULL)
                $file_path = $this->do_upload();
            else if(strlen($_FILES['fileToUpload']['name']) > 0){
                $file_path = $this->do_upload();
            }
            else
                $file_path = $this->AgentProfileModel->get_details()[0]->image;
            $this->AgentProfileModel->update_details($file_path);
        }
    }
